...
static ngx_int_t ngx_http_image_water_body_filter(ngx_http_request_t *r,
		ngx_chain_t *in);
static ngx_uint_t ngx_http_image_water_test(ngx_http_request_t *r,
		ngx_chain_t *in);
static ngx_int_t ngx_http_image_water_read(ngx_http_request_t *r,
		ngx_chain_t *in);
...
static char *ngx_http_image_water_filter_merge_conf(ngx_conf_t *cf,
...
static char *ngx_http_image_water_filter(ngx_conf_t *cf, ngx_command_t *cmd,
		void *conf);
...
{
{ ngx_string("image_water"), NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
		ngx_http_image_water_filter, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
{ ngx_string("image_buffer_size"), NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
		ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_image_water_filter_conf_t, buffer_size), NULL },
		ngx_null_command };
...
{ NGX_MODULE_V1, &ngx_http_image_water_filter_module_ctx, /* module context */
...
static ngx_buf_t *ngx_http_image_water_process(ngx_http_request_t *r)
{
	...
	status = MagickReadImageBlob(magick_wand_water, conf->image_water,
			conf->len);
	if (status == MagickFalse)
	{
		magick_wand_src = DestroyMagickWand(magick_wand_src);
	...
	if (width > 200)
	{
		img = GetImageFromMagickWand(magick_wand_src);
		...
		status = MagickCompositeImage(magick_wand_src, magick_wand_water,
				DissolveCompositeOp, geometry.x, geometry.y);
		if (status == MagickFalse)
		{
			magick_wand_src = DestroyMagickWand(magick_wand_src);
			magick_wand_water = DestroyMagickWand(magick_wand_water);
		...
		void *conf)
{
	...
	imcf->image_water = mmap(NULL, imcf->len - pa_offset, PROT_READ, MAP_SHARED,
			fd, pa_offset);
	...
	failed: ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "open file error \"%V\"",
			&value[i]);
